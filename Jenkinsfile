/* Variables to used inside the pipeline */
	...
	stages {
		stage('Install git if it does not exist') {
			steps {
					...
					if(gitExecutable == 0) {
						echo 'A git executable was not found on the server. Installng Git on the server.'
		...
		stage('Install Node.js for installing the linter package') {
			steps {
				...
				if(nodeExecutable == 0 && npmExecutable == 0) {
					...
					/* Change the script to always use the latest version of LTS release for Node */
				}
		...
		stage('Lint the markdown file') {
			steps {
				/* Link the markdown file in this repository */
